...
console.log(chalk.cyan(`<<<===========>>> ${chalk.green("Welcome to Ayaz Ahmed To-Do-App Project. ")} <<<=============>>> ${"\n \t\t <<<==========================>>>"}`));
let main = async () => {
...
let viewList = async () => {
    console.log(chalk.cyan(`n\ \t\t **************** Todo List ******************`));
...
let updateTask = async () => {
    ...
    console.log(chalk.yellowBright(` n\ \t\t Your Task ${fourthTask.Upadate} Updated Successfully.`));
    console.log(chalk.cyan(`\t [for view list please check a : view list Shukriya]\n`));
    todos;
};
let Exit = () => {
    condition = false;
    console.log(chalk.green("Thanks Duaon me yadd rakhna Allah Hafiz. \n Engineer Ayaz Ahmed Niaz Ahmed Laghari."));
};
main();
